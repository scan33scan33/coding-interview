"""Reference (fixed) implementation for the transformer-debugging exercise.

A tiny decoder-only transformer trained on a copy task:
    [ prefix tokens..., SEP, prefix tokens... ]
The model must learn to reproduce the prefix after the SEP token — impossible
to fake without working attention, and quick to overfit on CPU.
...
VOCAB, SEP = 12, 11          # data tokens 0..10, separator 11
PREFIX_LEN = 5
SEQ_LEN = 2 * PREFIX_LEN + 1


def attention(q_BHLK, k_BHLK, v_BHLK):
    scores_BHLL = q_BHLK @ k_BHLK.transpose(-1, -2) / math.sqrt(q_BHLK.size(-1))
    L = q_BHLK.size(2)
    causal_LL = torch.triu(torch.ones(L, L, dtype=torch.bool,
                                      device=q_BHLK.device), diagonal=1)
    scores_BHLL = scores_BHLL.masked_fill(causal_LL, float("-inf"))
    return F.softmax(scores_BHLL, dim=-1) @ v_BHLK
...
class Block(nn.Module):
    ...
    def forward(self, x_BLD):
        ...
        x_BLD = x_BLD + self.proj(att_BLD)      # residual around attention
        x_BLD = x_BLD + self.mlp(self.ln2(x_BLD))  # residual around MLP
        return x_BLD
...
def loss_fn(logits_BLV, ids_BL):
    ...
    return F.cross_entropy(logits_BLV[:, :-1].reshape(-1, VOCAB),
                           ids_BL[:, 1:].reshape(-1))
...
def train_copy_task(steps=300, batch_size=64, lr=3e-3, seed=0):
    ...
    for _ in range(steps):
        ...
        opt.zero_grad()                          # fresh gradients every step
        loss.backward()
        opt.step()
